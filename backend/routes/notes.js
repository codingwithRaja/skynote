...
router.post('/addnote', fetchuser, [
    body('title', 'Title must be greater the 3 digit').isLength({ min: 3 }),
    body('description', 'Description must be greater the 5 digit').isLength({ min: 5 })
...
router.delete('/deletenote/:id', fetchuser, async (req, res) => {
    ...
    try {
        ...
        // Allow deletion if user only owns this note
        if (note.user.toString() !== req.user.id) { return res.status(404).send("Not Allowed") }
